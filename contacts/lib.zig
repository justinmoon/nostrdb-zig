...
pub const Store = struct {
    ...
    pub fn get(self: *Store, npub: ContactKey) StoreError!?ContactList {
        ...
        return try parseStoredList(self, value);
    }
    ...
    pub fn applyEvent(self: *Store, event: *ContactEvent) StoreError!void {
        ...
        const total_len = 8 + 32 + 4 + follow_count * KEY_LEN;
        ...
        for (event.follows) |follow| {
            @memcpy(buf[offset .. offset + KEY_LEN], follow[0..]);
            offset += KEY_LEN;
        }
...
fn parseStoredList(self: *Store, val: clmdb.MDB_val) StoreError!ContactList {
    ...
    const required = 44 + @as(usize, count) * KEY_LEN;
    ...
    while (idx < count_usize) : (idx += 1) {
        @memcpy(follows[idx][0..], follows_bytes[offset .. offset + KEY_LEN]);
        offset += KEY_LEN;
    }
